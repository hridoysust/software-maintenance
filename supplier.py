...
#-----------------------------------------------------------------------------------------------------
    def add(self):
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row = self.db.fetch("SELECT * FROM supplier WHERE invoice=?", (self.var_sup_invoice.get(),))
                if row:
                    messagebox.showerror("Error","Invoice no. is already assigned",parent=self.root)
                else:
                    self.db.execute("INSERT INTO supplier(invoice,name,contact,desc) VALUES(?,?,?,?)", (
    ...
    def show(self):
        try:
            rows = self.db.fetch("SELECT * FROM supplier")
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if not row:
                    ...
                    self.db.execute("UPDATE supplier SET name=?, contact=?, desc=? WHERE invoice=?", (
                        self.var_name.get(),
                        self.var_contact.get(),
                        self.txt_desc.get('1.0',END),
                        self.var_sup_invoice.get(),
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                if not row:
                    ...
                    if op==True:
                        self.db.execute("DELETE FROM supplier WHERE invoice=?", (self.var_sup_invoice.get(),))
